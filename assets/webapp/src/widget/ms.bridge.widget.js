    ...
    window.MsJsBridge = {
        ...
        callback: function (event, data) {
            ...
            if (event === BridgeMethod.confirm) {
                if (jsonData.success) {
                    if (jsonData.data === "Y") {
                        opts.okCallback && opts.okCallback.call(opts.scope);
                    }
                    else {
                        opts.cancelCallback && opts.cancelCallback.call(opts.scope);
                    }
                }
                else {
                    opts.cancelCallback && opts.cancelCallback.call(opts.scope);
            ...
            if (event === BridgeMethod.showToast) {
                opts.callback && opts.callback.call(opts.scope);
                return;
            }
            ...
            if (jsonData.success) {
                opts.callback && opts.callback.call(opts.scope, jsonData.data);
            }
            else {
                opts.errorCallback && opts.errorCallback.call(opts.scope, jsonData.data);
            }
        },
        ...
        checkUpdate: function (opts) {
            ...
            if (window.MsJsBridge.isReady(BridgeMethod.checkUpdate)) {
                ...
                opts.errorCallback && opts.errorCallback.call(opts.scope);
            }
        },
        //checkUpdateCallback
        checkUpdateCallback: function (data, opts) {
            if (data.success) {
                if (data.data == "YES") {
                    opts.successCallback.call(opts.scope);
                }
                else {
                    opts.errorCallback.call(opts.scope);
                }
            }
            else {
                opts.errorCallback.call(opts.scope);
